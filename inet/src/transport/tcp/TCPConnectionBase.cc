...
TCPConnection::TCPConnection(TCP *_mod, int _appGateIndex, int _connId)
{
    ...
    bool sw = false;
    if(tcpMain != NULL)
        sw = tcpMain->recordStatistics;
    if(tcpMain2 != NULL)
        sw = tcpMain2->recordStatistics;
    if (sw)
    {
...
TCPConnection::TCPConnection(TCP2 *_mod, int _appGateIndex, int _connId)
{
    tcpMain = NULL;
    tcpMain2 = _mod;
    appGateIndex = _appGateIndex;
...
bool TCPConnection::processAppCommand(cMessage *msg)
{
    ...
    switch (event)
    {
        ...
        default:
            ...
            if(tcpMain2 != NULL)
                throw cRuntimeError(tcpMain2, "wrong event code");
    }
    ...
    switch (commandCode)
    {
        ...
        default:
            if(tcpMain != NULL)
                ...
                throw cRuntimeError(tcpMain2, "Unknown message kind in app command");
    }
}
...
bool TCPConnection::performStateTransition(const TCPEventCode& event)
{
    ...
    if (oldState != fsm.getState())
    {
        ...
        if(tcpMain2 != NULL)
            testingEV << tcpMain2->getName() << ": " << stateName(oldState) << " --> " << stateName(fsm.getState()) << "  (on " << eventName(event) << ")\n";
