...
void TCPConnection::segmentArrivalWhileClosed(TCPSegment *tcpseg, IPvXAddress srcAddr, IPvXAddress destAddr)
{
    ...
    if(tcpMain!=NULL)
        ...
        tcpEV1 << "Segment doesn't belong to any existing connection\n";
    ...
    if (tcpseg->getRstBit())
    {
        if(tcpMain!=NULL)
            ...
            tcpEV1 << "RST bit set: dropping segment\n";
        return;
    }
    ...
    if (!tcpseg->getAckBit())
    {
        if(tcpMain!=NULL)
            ...
            tcpEV1 << "ACK bit not set: sending RST+ACK\n";
    ...
    else
    {
        if(tcpMain!=NULL)
            ...
            tcpEV1 << "ACK bit set: sending RST\n";
        ...
        if (state->ts_enabled)
        {
                    ...
                    (simTime() - state->time_last_data_sent) > PAWS_IDLE_TIME_THRESH) // PAWS_IDLE_TIME_THRESH = 24 days
            {
                if(tcpMain!=NULL)
                    ...
                    tcpEV1 << "PAWS: Segment is not acceptable, TSval=" << tsval << " in " <<
                        stateName(fsm.getState()) << " state received: dropping segment\n";
                acceptable = false;
            }
        }
    ...
    if (tcpseg->getSynBit())
    {
        ...
        if (state->fork)
        {
            ...
            if(tcpMain!=NULL)
                ...
                tcpMain2->addForkedConnection(this, conn, destAddr, srcAddr, tcpseg->getDestPort(), tcpseg->getSrcPort());
            if(tcpMain!=NULL)
        ...
        else
        {
            if(tcpMain!=NULL)
                ...
                tcpMain2->updateSockPair(this, destAddr, srcAddr, tcpseg->getDestPort(), tcpseg->getSrcPort());
        }
