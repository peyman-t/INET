...
TCPConnection *TCPConnection::cloneListeningConnection()
{
    ...
    if(tcpMain != NULL)
        ...
        conn = new TCPConnection(tcpMain2, appGateIndex, connId);
...
void TCPConnection::sendToIP(TCPSegment *tcpseg)
{
    ...
    if (!remoteAddr.isIPv6())
    {
        ...
        if(tcpMain != NULL)
            controlInfo->setProtocol(tcpMain->type);
        else
            controlInfo->setProtocol(tcpMain2->type);
...
void TCPConnection::sendToApp(cMessage *msg)
{
    if(tcpMain != NULL)
        ...
        tcpMain2->send(msg, "appOut", appGateIndex);
}
...
void TCPConnection::initConnection(TCPOpenCommand *openCmd)
{
    ...
    if(tcpMain != NULL)
        ...
        sendQueue = tcpMain2->createSendQueue(transferMode);
    ...
    if(tcpMain != NULL)
        ...
        receiveQueue = tcpMain2->createReceiveQueue(transferMode);
    ...
    if (!tcpAlgorithmClass || !tcpAlgorithmClass[0]) {
        if(tcpMain != NULL)
            ...
            tcpAlgorithmClass = tcpMain2->par("tcpAlgorithmClass");
    }
...
{
    long advertisedWindowPar;
    if(tcpMain != NULL) {
        advertisedWindowPar= tcpMain->par("advertisedWindow").longValue();
        state->ws_support = tcpMain->par("windowScalingSupport"); // if set, this means that current host supports WS (RFC 1323)
    } else {
        advertisedWindowPar= tcpMain2->par("advertisedWindow").longValue();
        state->ws_support = tcpMain2->par("windowScalingSupport"); // if set, this means that current host supports WS (RFC 1323)
    }
    ...
    if(tcpMain != NULL) {
        ...
        state->delayed_acks_enabled = tcpMain2->par("delayedAcksEnabled"); // delayed ACK algorithm (RFC 1122) enabled/disabled
        state->nagle_enabled = tcpMain2->par("nagleEnabled"); // Nagle's algorithm (RFC 896) enabled/disabled
        state->limited_transmit_enabled = tcpMain2->par("limitedTransmitEnabled"); // Limited Transmit algorithm (RFC 3042) enabled/disabled
        state->increased_IW_enabled = tcpMain2->par("increasedIWEnabled"); // Increased Initial Window (RFC 3390) enabled/disabled
        state->snd_mss = tcpMain2->par("mss").longValue(); // Maximum Segment Size (RFC 793)
        state->ts_support = tcpMain2->par("timestampSupport"); // if set, this means that current host supports TS (RFC 1323)
        state->sack_support = tcpMain2->par("sackSupport"); // if set, this means that current host supports SACK (RFC 2018, 2883, 3517)
    }
    ...
    if (state->sack_support)
    {
        ...
        if(tcpMain != NULL) {
            ...
            std::string algorithmName2 = tcpMain2->par("tcpAlgorithmClass");
            if (algorithmName1 != algorithmName2) // TODO add additional checks for new SACK supporting algorithms here once they are implemented
            {
                EV << "If you want to use TCP SACK please set tcpAlgorithmClass to TCPReno\n";
...
void TCPConnection::sendSyn()
{
    if (remoteAddr.isUnspecified() || remotePort == -1) {
        if(tcpMain != NULL)
            ...
            throw cRuntimeError(tcpMain2, "Error processing command OPEN_ACTIVE: foreign socket unspecified");
    }
    ...
    if (localPort == -1) {
        if(tcpMain != NULL)
            ...
            throw cRuntimeError(tcpMain2, "Error processing command OPEN_ACTIVE: local port unspecified");
    }
