...
Register_Class(ECNNUM);


ECNNUM::ECNNUM() : TCPTahoeRenoFamily(),
        state((TCPRenoStateVariables *&)TCPAlgorithm::state)
{
}

void ECNNUM::recalculateSlowStartThreshold()
{
    // RFC 2581, page 4:
...
void ECNNUM::processRexmitTimer(TCPEventCode& event)
{
...
//        state->ecnnum_Rate = (newCwnd * 8 / (double)state->minrtt.dbl()) / 1000 / state->snd_mss / 8;
        ...
        if(q == 0 || (1 / q ) > 2 * state->ecnnum_Rate)
...
void ECNNUM::processPaceTimer(TCPEventCode& event)
{
...
void ECNNUM::receivedDataAck(uint32 firstSeqAcked)
{
...
void ECNNUM::receivedDuplicateAck()
{
