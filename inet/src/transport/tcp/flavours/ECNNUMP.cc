...
Register_Class(ECNNUMP);


ECNNUMP::ECNNUMP() : TCPTahoeRenoFamily(),
        state((TCPRenoStateVariables *&)TCPAlgorithm::state)
{
}

void ECNNUMP::recalculateSlowStartThreshold()
{
    // RFC 2581, page 4:
...
void ECNNUMP::processRexmitTimer(TCPEventCode& event)
{
...
void ECNNUMP::processRateUpdateTimer(TCPEventCode& event)
{
...
//        double newRate = state->ecnnum_Rate + state->ecnnum_alpha * state->ecnnum_Rate * (1 / state->ecnnum_Rate - q );
        double newRate = state->ecnnum_Rate + state->ecnnum_alpha * (1 / state->ecnnum_Rate - q );
...
void ECNNUMP::processPaceTimer(TCPEventCode& event)
{
...
void ECNNUMP::receivedDataAck(uint32 firstSeqAcked)
{
...
void ECNNUMP::receivedDuplicateAck()
{
