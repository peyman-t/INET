...
ABC::ABC() : TCPTahoeRenoFamily(),
        state((TCPRenoStateVariables *&)TCPAlgorithm::state)
{
}

void ABC::recalculateSlowStartThreshold()
{
    // RFC 2581, page 4:
...
void ABC::processRexmitTimer(TCPEventCode& event)
{
...
void ABC::processPaceTimer(TCPEventCode& event)
{
...
void ABC::receivedDataAck(uint32 firstSeqAcked)
{
    ...
    else
    {
        ...
        if(state->ece) {
            state->snd_cwnd = state->snd_cwnd - state->snd_mss + incr;
